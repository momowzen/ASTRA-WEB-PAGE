...
const profileConverter = {
  ...
  fromFirestore: (snapshot: { data: () => Record<string, unknown>; id: string }): MemberProfile => {
    ...
    return {
      ...
      necklace: (data.necklace as boolean) ?? false,
      ring: (data.ring as boolean) ?? false,
      earring: (data.earring as boolean) ?? false,
      bracelet: (data.bracelet as boolean) ?? false,
      belt: (data.belt as boolean) ?? false,
